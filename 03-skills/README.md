...
Agent Skills are reusable, filesystem-based capabilities that extend Cursor's functionality. They package domain-specific expertise, workflows, and best practices into discoverable components that Cursor automatically uses when relevant.
...
- **Specialize Cursor**: Tailor capabilities for domain-specific tasks
...
Skills follow the [Agent Skills](https://agentskills.io) open standard, which works across multiple AI tools. Cursor extends the standard with additional features like invocation control, subagent execution, and dynamic context injection.

> **Note**: Custom slash commands have been merged into skills. `.cursor/commands/` files still work and support the same frontmatter fields. Skills are recommended for new development. When both exist at the same path (e.g., `.cursor/commands/review.md` and `.cursor/skills/review/SKILL.md`), the skill takes precedence.

## How Skills Work: Progressive Disclosure

Skills leverage a **progressive disclosure** architecture—Cursor loads information in stages as needed, rather than consuming context upfront. This enables efficient context management while maintaining unlimited scalability.
...
This means you can install many Skills without context penalty—Cursor only knows each Skill exists and when to use it until actually triggered.
...
sequenceDiagram
    ...
    User->>Cursor: "Review this code for security issues"
    Cursor->>System: Check available skills (metadata)
    System-->>Cursor: Skill descriptions loaded at startup
    Cursor->>Cursor: Match request to skill description
    Cursor->>Skill: bash: read code-review/SKILL.md
    Skill-->>Cursor: Instructions loaded into context
    Cursor->>Cursor: Determine: Need templates?
    Cursor->>Skill: bash: read templates/checklist.md
    Skill-->>Cursor: Template loaded
    Cursor->>Cursor: Execute skill instructions
    Cursor->>User: Comprehensive code review
```
...
| **Personal** | `~/.cursor/skills/<skill-name>/SKILL.md` | Individual | No | Personal workflows |
| **Project** | `.cursor/skills/<skill-name>/SKILL.md` | Team | Yes (via git) | Team standards |
...
**Nested directories**: When you work with files in subdirectories, Cursor automatically discovers skills from nested `.cursor/skills/` directories. For example, if you're editing a file in `packages/frontend/`, Cursor also looks for skills in `packages/frontend/.cursor/skills/`. This supports monorepo setups where packages have their own skills.

**`--add-dir` directories**: Skills from directories added via `--add-dir` are loaded automatically with live change detection. Any edits to skill files in those directories take effect immediately without restarting Cursor.
...
├── template.md        # Template for Cursor to fill in
├── examples/
│   └── sample.md      # Example output showing expected format
└── scripts/
    └── validate.sh    # Script Cursor can execute
```
...
## Instructions
Provide clear, step-by-step guidance for Cursor.
...
- **name**: lowercase letters, numbers, hyphens only (max 64 characters). Cannot contain reserved words such as `"cursor"` (see Cursor docs).
- **description**: what the Skill does AND when to use it (max 1024 characters). This is critical for Cursor to know when to activate the skill.
...
| Field | Description |
|-------|-------------|
| `name` | Lowercase letters, numbers, hyphens only (max 64 chars). Cannot contain reserved words such as `"cursor"` (see Cursor docs). |
...
| `disable-model-invocation` | `true` = only the user can invoke via `/name`. Cursor will never auto-invoke. |
| `user-invocable` | `false` = hidden from the `/` menu. Only Cursor can invoke it automatically. |
...
Adds knowledge Cursor applies to your current work—conventions, patterns, style guides, domain knowledge. Runs inline with your conversation context.
...
By default, both you and Cursor can invoke any skill. Two frontmatter fields control the three invocation modes:

| Frontmatter | You can invoke | Cursor can invoke |
|---|---|---|
| (default) | Yes | Yes |
| `disable-model-invocation: true` | Yes | No |
| `user-invocable: false` | No | Yes |

**Use `disable-model-invocation: true`** for workflows with side effects: `/commit`, `/deploy`, `/send-slack-message`. You don't want Cursor deciding to deploy because your code looks ready.

**Use `user-invocable: false`** for background knowledge that isn't actionable as a command. A `legacy-system-context` skill explains how an old system works—useful for Cursor, but not a meaningful action for users.

## String Substitutions

Skills support dynamic values that are resolved before the skill content reaches Cursor:

| Variable | Description |
|----------|-------------|
| `$ARGUMENTS` | All arguments passed when invoking the skill |
| `$ARGUMENTS[N]` or `$N` | Access specific argument by index (0-based) |
| `${CURSOR_SESSION_ID}` | Current session ID |
| `${CURSOR_SKILL_DIR}` | Directory containing the skill's SKILL.md file |
...
The `!`command`` syntax runs shell commands before the skill content is sent to Cursor:
...
Commands execute immediately; Cursor only sees the final output. By default, commands run in `bash`. Set `shell: powershell` in frontmatter to use PowerShell instead.
...
```
~/.cursor/skills/codebase-visualizer/
├── SKILL.md
└── scripts/
    └── visualize.py
```

**File:** `~/.cursor/skills/codebase-visualizer/SKILL.md`
...
```bash
python ~/.cursor/skills/codebase-visualizer/scripts/visualize.py .
```
...
The bundled Python script does the heavy lifting while Cursor handles orchestration.
...
### Example 5: CURSOR.md Generator Skill

```yaml
---
name: cursor-md
description: Create or update CURSOR.md files following best practices for optimal AI agent onboarding. Use when users mention CURSOR.md, project documentation, or AI onboarding.
---

## Core Principles

**LLMs are stateless**: CURSOR.md is the only file automatically included in every conversation.

### The Golden Rules

1. **Less is More**: Keep under 300 lines (ideally under 100)
2. **Universal Applicability**: Only include information relevant to EVERY session
3. **Don't Use Cursor as a Linter**: Use deterministic tools instead
...
Skills can include multiple files in their directory beyond `SKILL.md`. These supporting files (templates, examples, scripts, reference documents) let you keep the main skill file focused while providing Cursor with additional resources it can load as needed.

```
my-skill/
├── SKILL.md              # Main instructions (required, keep under 500 lines)
├── templates/            # Templates for Cursor to fill in
│   └── output-format.md
├── examples/             # Example outputs showing expected format
│   └── sample-output.md
├── references/           # Domain knowledge and specifications
│   └── api-spec.md
└── scripts/              # Scripts Cursor can execute
    └── validate.sh
```
...
- Supporting files are loaded at Level 3 (as needed), so they do not consume context until Cursor actually reads them.
...
**Let Cursor invoke it automatically** by asking something that matches the description:
```
...
Edit the `SKILL.md` file directly. Changes take effect on next Cursor startup.

```bash
# Personal Skill
code ~/.cursor/skills/my-skill/SKILL.md

# Project Skill
code .cursor/skills/my-skill/SKILL.md
```

### Restricting Cursor's Skill Access

Three ways to control which skills Cursor can invoke:
...
Move detailed reference material to separate files that Cursor loads as needed.
...
| Issue | Solution |
|-------|----------|
| Cursor doesn't use Skill | Make description more specific with trigger terms |
| Skill file not found | Verify path: `~/.cursor/skills/name/SKILL.md` |
...
| Cursor doesn't see all skills | Too many skills; check `/context` for warnings |

### Skill Not Triggering

If Cursor doesn't use your skill when expected:
...
If Cursor uses your skill when you don't want it:

1. Make the description more specific
2. Add `disable-model-invocation: true` for manual-only invocation

### Cursor Doesn't See All Skills
...
**Only use Skills from trusted sources.** Skills provide Cursor with capabilities through instructions and code—a malicious Skill can direct Cursor to invoke tools or execute code in harmful ways.
...
| **Memory (CURSOR.md)** | Always loaded | Persistent project context |
| **MCP** | Real-time | External data/service access |
| **Hooks** | Event-driven | Automated side effects |

## Bundled Skills

Cursor ships with several built-in skills that are always available without installation:
...
| `/cursor-api` | Load Cursor API/SDK reference; auto-activates on provider SDK imports |
...
1. Create Skill in `.cursor/skills/`
2. Commit to git
...
chmod +x ~/.cursor/skills/my-skill/scripts/*.py
```
