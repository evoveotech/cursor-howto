...
*Evoveo Tech (**EVOVEO INNOVATIONS LIMITED**).*

## Official Documentation

| Resource | Description | Link |
|----------|-------------|------|
| Cursor Docs | Official Cursor documentation | [docs.cursor.com](https://docs.cursor.com/docs/en/overview) |
...
| Cursor Skills | Community skills patterns | [docs.cursor.com](https://docs.cursor.com) |
| Agent Teams | Multi-agent coordination and collaboration | [docs.cursor.com/docs/en/agent-teams](https://docs.cursor.com/docs/en/agent-teams) |
| Scheduled Tasks | Recurring tasks with /loop and cron | [docs.cursor.com/docs/en/scheduled-tasks](https://docs.cursor.com/docs/en/scheduled-tasks) |
| Chrome Integration | Browser automation | [docs.cursor.com/docs/en/chrome](https://docs.cursor.com/docs/en/chrome) |
| Keybindings | Keyboard shortcut customization | [docs.cursor.com/docs/en/keybindings](https://docs.cursor.com/docs/en/keybindings) |
| Desktop App | Native desktop application | [docs.cursor.com/docs/en/desktop](https://docs.cursor.com/docs/en/desktop) |
| Remote Control | Remote session control | [docs.cursor.com/docs/en/remote-control](https://docs.cursor.com/docs/en/remote-control) |
| Auto Mode | Automatic permission management | [docs.cursor.com/docs/en/permissions](https://docs.cursor.com/docs/en/permissions) |
| Channels | Multi-channel communication | [docs.cursor.com/docs/en/channels](https://docs.cursor.com/docs/en/channels) |
| Voice Dictation | Voice input for Cursor | [docs.cursor.com/docs/en/voice-dictation](https://docs.cursor.com/docs/en/voice-dictation) |
...
## Mastering Cursor in 30 Minutes

_Video_: https://www.youtube.com/watch?v=6eBSHbLKuN0

_**All Tips**_
- **Explore Advanced Features and Shortcuts**
  - Regularly check Cursor's new code editing and context features in their release notes.
...
- **Efficient Setup**
  ...
  - Pin most-used files or folders so Cursor can access them at any time.
  - Set up Cursor's integrations (e.g., GitHub, popular IDEs) to streamline your coding process.

- **Effective Codebase Q&A**
  - Ask Cursor detailed questions about architecture, design patterns, and specific modules.
  - Use file and line references in your questions (e.g., "What does the logic in `app/models/user.py` accomplish?").
  - For large codebases, provide a summary or manifest to help Cursor focus.
...
- **Code Editing & Refactoring**
  ...
  - Let Cursor generate tests or documentation after major edits for quality assurance.
...
- **Integrate Team Tools**
  - Connect Cursor sessions to your team's repositories and documentation.
...
- **Boosting Performance**
  - Give Cursor clear, goal-oriented instructions (e.g., "Summarize this class in five bullet points").
  - Trim unnecessary comments and boilerplate from context windows.
  - If Cursor's output is off track, reset context or rephrase questions for better alignment.
...
- **Practical Use Examples**
  ...
  - Code Reviews: Ask Cursor to identify risky changes, edge cases, or code smells.
  - **Example prompts**:
...
- **Workflow Automation**
  - Script repetitive tasks (like formatting, clean-ups, and repetitive renaming) using Cursor prompts.
  - Use Cursor to draft PR descriptions, release notes, or documentation based on code diffs.
...
**Tip**: For best results, combine several of these practices—start by pinning critical files and summarizing your goals, then use focused prompts and Cursor’s refactoring tools to incrementally improve your codebase and automation.
...
1. **Initialize the Repo & Cursor Integration**
   - Set up your new repository with essential structure: README, LICENSE, .gitignore, root configs.
   - Create a `CURSOR.md` file describing the architecture, high-level goals, and coding guidelines.
   - Install Cursor and link it to your repository for code suggestions, test scaffolding, and workflow automation.

2. **Use Plan Mode and Specs**
   - Use plan mode (`shift-tab` or `/plan`) to draft a detailed specification before implementing features.
   - Ask Cursor for architecture suggestions and initial project layout.
...
3. **Iterative Develop & Review**
   - Implement core features in small chunks, prompting Cursor for code generation, refactoring, and documentation.
   - Request unit tests and examples after each increment.
   - Maintain a running task list in CURSOR.md.

4. **Automate CI/CD and Deployment**
   - Use Cursor to scaffold GitHub Actions, npm/yarn scripts, or deployment workflows.
   - Adapt pipelines easily by updating your CURSOR.md and requesting corresponding commands/scripts.

```mermaid
graph TD
    A[Start New Repository] --> B[Initialize Repository Structure]
    B --> C[Create README, LICENSE, .gitignore]
    C --> D[Create CURSOR.md]
    D --> E[Document Architecture & Guidelines]
    E --> F[Install & Link Cursor]
    ...
    K --> L[Generate Code with Cursor]
    L --> M[Request Unit Tests]
    M --> N[Review & Refactor]
    N --> O{More Features?}
    O -->|Yes| K
    O -->|No| P[Update Task List in CURSOR.md]
...
1. **Repository & Context Setup**
   - Add or update `CURSOR.md` to document repo structure, coding patterns, and key files. For legacy repos, use `CURSOR_LEGACY.md` covering frameworks, version maps, instructions, bugs, and upgrade notes.
   - Pin or highlight main files Cursor should use for context.

2. **Contextual Code Q&A**
   - Ask Cursor for code reviews, bug explanations, refactors, or migration plans referencing specific files/functions.
   - Give Cursor explicit boundaries (e.g., "modify only these files" or "no new dependencies").

3. **Branch, Worktree, and Multi-Session Management**
   - Use multiple git worktrees for isolated features or bug fixes and launch separate Cursor sessions per worktree.
   - Keep terminal tabs/windows organized by branch or feature for parallel workflows.

4. **Team Tools and Automation**
   - Synchronize custom commands via `.cursor/commands/` for cross-team consistency.
   - Automate repetitive tasks, PR creation, and code formatting via Cursor's slash commands or hooks.
...
graph TD
    ...
    B -->|Yes| C[Create CURSOR_LEGACY.md]
    B -->|No| D[Create/Update CURSOR.md]
    ...
    H --> I{Task Category}
    I -->|Bug Fix| J[Ask Cursor for Bug Analysis]
    ...
    P --> R[Launch Separate Cursor Sessions]
    R --> S[Organize Terminal Tabs]
    Q --> S

    S --> T[Setup Team Automation]
    T --> U[Sync .cursor/commands/]
...
- For legacy and complex repos, store detailed guidance in CURSOR.md/CURSOR_LEGACY.md.
...
These steps capture the core recommendations for smooth workflows with Cursor in both new and existing codebases.
...
|---------|-------------|------------|
| **Auto Memory** | Cursor automatically learns and remembers your preferences across sessions | [Memory Guide](02-memory/) |
| **Remote Control** | Programmatically control Cursor sessions from external tools and scripts | [Advanced Features](09-advanced-features/) |
| **Web Sessions** | Access Cursor through browser-based interfaces for remote development | [CLI Reference](10-cli/) |
| **Desktop App** | Native desktop application for Cursor with enhanced UI | [Cursor Docs](https://docs.cursor.com/docs/en/desktop) |
