...
Checkpoints allow you to save conversation state and rewind to previous points in your Cursor session. This is invaluable for exploring different approaches, recovering from mistakes, or comparing alternative solutions.
...
Cursor automatically creates checkpoints for you:
...
Cursor: I'll add Redis caching to your API endpoints...
[Makes changes at checkpoint A]

User: Actually, let's try in-memory caching instead

Cursor: I'll rewind to explore a different approach...
...
Cursor: I'll refactor the authentication module...
[Makes extensive changes]

User: Wait, that broke the OAuth integration. Let's go back.

Cursor: I'll help you rewind to before the refactoring...
...
Cursor: [Makes experimental changes]

User: The tests are failing. Let's rewind.
[User presses Esc+Esc and rewinds to the checkpoint]

Cursor: I've rewound the changes. Let's try a different approach.
```
...
Cursor automatically manages your checkpoints:
...
- **External changes NOT tracked** - Changes made outside Cursor (in your editor, terminal, etc.) are not captured
...
1. **Work normally** - Cursor creates checkpoints automatically
...
Checkpoints let you go back — but how do you know *when* you should? As your conversation grows, Cursor's context window fills up and model quality silently degrades. You might be shipping code from a half-blind model without realizing it.

**cc-context-stats** (example extension pattern) solves this by adding real-time **context zones** to your Cursor status bar. It tracks where you are in the context window — from **Plan** (green, safe to plan and code) through **Code** (yellow, avoid starting new plans) to **Dump** (orange, finish up and rewind). When you see the zone shift, you know it's time to checkpoint and start fresh instead of pushing through with degraded output. Related tooling may be published under **[Evoveo Tech](https://github.com/evoveotech)**.
...
- [Official Checkpointing Documentation](https://docs.cursor.com/docs/en/checkpointing)
- [Advanced Features Guide](../09-advanced-features/) - Extended thinking and other capabilities

## Summary

Checkpoints are an automatic feature in Cursor that lets you safely explore different approaches without fear of losing work. Every user prompt creates a new checkpoint automatically, so you can rewind to any previous point in your session.
