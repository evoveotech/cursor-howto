...
**New to Cursor?** This guide helps you master Cursor features at your own pace. Whether you're a complete beginner or an experienced developer, start with the self-assessment quiz below to find the right path for you.
...
- [ ] I can start Cursor and have a conversation (`cursor`)
- [ ] I have created or edited a CURSOR.md file
...
- [ ] I have set up hooks in ~/.cursor/settings.json
- [ ] I have created or used custom subagents (.cursor/agents/)
- [ ] I have used print mode (`cursor -p`) for scripting or CI/CD
...
> **Interactive version**: Run `/self-assessment` in Cursor for a guided, interactive quiz that scores your proficiency across all 10 feature areas and generates a personalized learning path.
...
✅ Understand how Cursor loads context automatically
...
# In Cursor, type: /optimize
```

#### Success Criteria
- [ ] Successfully invoke `/optimize` command
- [ ] Cursor remembers your project standards from CURSOR.md
...
> **Check your understanding**: Run `/lesson-quiz slash-commands` or `/lesson-quiz memory` in Cursor to test what you've learned.
...
cursor "explain this project"           # Interactive mode
cursor -p "explain this function"       # Print mode (non-interactive)
...
- [ ] Piped a file to Cursor for analysis
...
- [ ] Have set up project memory via CURSOR.md ([02-memory/](02-memory/))
- [ ] Know how to create and restore checkpoints ([08-checkpoints/](08-checkpoints/))
- [ ] Can use `cursor` and `cursor -p` from the command line ([10-cli/](10-cli/))
...
cp 06-hooks/pre-tool-check.sh ~/.cursor/hooks/
chmod +x ~/.cursor/hooks/pre-tool-check.sh
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "~/.cursor/hooks/pre-tool-check.sh"
          }
        ]
      }
    ]
...
# In Cursor: /mcp__github__list_prs
...
2. Let Cursor delegate review to code-reviewer subagent
3. Use hooks to run tests automatically

#### Success Criteria
- [ ] Successfully query GitHub data via MCP
- [ ] Cursor delegates complex tasks to subagents
...
cursor --permission-mode plan "analyze this codebase"
cursor --permission-mode acceptEdits "refactor the auth module"
cursor --permission-mode auto "implement the feature"
...
cursor --permission-mode auto "implement user settings page"

# Exercise 6: Enable agent teams
export CURSOR_AGENT_TEAMS=1
# Ask Cursor: "Implement feature X using a team approach"
...
# Use voice input for hands-free interaction with Cursor
```
...
✅ Set up CI/CD integration with `cursor -p`
...
# In Cursor: /plugin install pr-review

# Exercise 2: Print mode for CI/CD
cursor -p "Run all tests and generate report"
...
cursor -p --max-turns 3 --output-format json "review code"

# Exercise 6: Batch processing
for file in *.md; do
  cursor -p --output-format json "summarize this: $(cat $file)" > ${file%.md}.summary.json
done
```

#### CI/CD Integration Exercise
Create a simple CI/CD script:
1. Use `cursor -p` to review changed files
...
This repository includes two interactive skills you can use anytime in Cursor to evaluate your understanding:
...
1. Copy one slash command: `cp 01-slash-commands/optimize.md .cursor/commands/`
2. Try it in Cursor: `/optimize`
...
2. **Project memory** (15 min): Create CURSOR.md with your project standards
...
**Outcome**: You'll be a Cursor power user ready to train others and automate complex workflows
...
- Join Cursor community discussions
...
- [ ] Set up print mode (`cursor -p`) CI/CD
- [ ] Created JSON output for automation
- [ ] Integrated Cursor into CI/CD pipeline
- [ ] Created team plugin
...
1. **Create team documentation** - Document your team's Cursor setup
2. **Build custom plugins** - Package your team's workflows
3. **Explore Remote Control** - Control Cursor sessions programmatically from external tools
4. **Try Web Sessions** - Use Cursor through browser-based interfaces for remote development
5. **Use the Desktop App** - Access Cursor features through the native desktop application
6. **Use Auto Mode** - Let Cursor work autonomously with a background safety classifier
7. **Leverage Auto Memory** - Let Cursor learn your preferences automatically over time
...
10. **Try Voice Dictation** - Use hands-free voice input for interaction with Cursor
...
15. **Stay updated** - Follow Cursor releases and new features
...
**Maintained by**: [Evoveo Tech](https://github.com/evoveotech) (EVOVEO INNOVATIONS LIMITED)  
