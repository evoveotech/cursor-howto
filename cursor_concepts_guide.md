...
# Complete Guide to Cursor Concepts
...
Slash commands are user-invoked shortcuts stored as Markdown files that Cursor can execute. They enable teams to standardize frequently-used prompts and workflows.

### Architecture

```mermaid
graph TD
    A["User Input: /command-name"] -->|Triggers| B["Search .cursor/commands/"]
    B -->|Finds| C["command-name.md"]
    C -->|Loads| D["Markdown Content"]
    D -->|Executes| E["Cursor Processes Prompt"]
...
```mermaid
graph LR
    A["Project Root"] -->|contains| B[".cursor/commands/"]
...
|----------|-------|--------------|----------|-------------|
| `.cursor/commands/` | Project-specific | Team members | Team workflows, shared standards | ✅ Yes |
| `~/.cursor/commands/` | Personal | Individual user | Personal shortcuts across projects | ❌ No |
...
**File:** `.cursor/commands/optimize.md`
...
# Cursor loads the prompt and waits for code input
```
...
# Cursor runs through checklist and prepares the PR
```

#### Example 3: Hierarchical Documentation Generator

**File:** `.cursor/commands/docs/generate-api-docs.md`
...
sequenceDiagram
    ...
    User->>Cursor: Types /optimize
    Cursor->>FS: Searches .cursor/commands/
    FS-->>Cursor: Returns optimize.md
    Cursor->>Cursor: Loads Markdown content
    Cursor->>User: Displays prompt context
    User->>Cursor: Provides code to analyze
    Cursor->>CLI: (May execute scripts)
    CLI-->>Cursor: Results
    Cursor->>User: Returns analysis
```
...
**File:** `.cursor/agents/test-engineer.md`
...
**File:** `.cursor/agents/documentation-writer.md`
...
### Memory Hierarchy in Cursor (7 Tiers)

Cursor loads memory from 7 tiers, listed from highest to lowest priority:

```mermaid
graph TD
    A["1. Managed Policy<br/>Enterprise admin policies"] --> B["2. Project Memory<br/>./CURSOR.md"]
    B --> C["3. Project Rules<br/>.cursor/rules/*.md"]
    C --> D["4. User Memory<br/>~/.cursor/CURSOR.md"]
    D --> E["5. User Rules<br/>~/.cursor/rules/*.md"]
    E --> F["6. Local Memory<br/>.cursor/local/CURSOR.md"]
...
| 2. Project | `./CURSOR.md` | Project | High | Team (Git) | Team standards, architecture |
| 3. Project Rules | `.cursor/rules/*.md` | Project | High | Team (Git) | Modular project conventions |
| 4. User | `~/.cursor/CURSOR.md` | Personal | Medium | Individual | Personal preferences |
| 5. User Rules | `~/.cursor/rules/*.md` | Personal | Medium | Individual | Personal rule modules |
| 6. Local | `.cursor/local/CURSOR.md` | Local | Low | Not shared | Machine-specific settings |
| 7. Auto Memory | Automatic | Session | Lowest | Individual | Learned preferences, patterns |

### Auto Memory

Auto Memory automatically captures user preferences and patterns observed during sessions. Cursor learns from your interactions and remembers:
...
MCP (Model Context Protocol) is a standardized way for Cursor to access external tools, APIs, and real-time data sources. Unlike Memory, MCP provides live access to changing data.
...
```mermaid
graph TB
    A["Cursor"] -->|MCP| B["Filesystem<br/>MCP Server"]
...
sequenceDiagram
    ...
    User->>Cursor: Type /mcp
    Cursor->>Cursor: List available MCP servers
    Cursor->>User: Show options
    User->>Cursor: Select GitHub MCP
    Cursor->>Config: Update configuration
    Config->>Cursor: Activate connection
    Cursor->>Service: Test connection
    Service-->>Cursor: Authentication successful
    Cursor->>User: ✅ MCP connected!
```
...
**File:** `.mcp.json` (project scope) or `~/.cursor/mcp.json` (user scope)
...
Cursor: I'll query your database to find that information.
...
Agent Skills are reusable, model-invoked capabilities packaged as folders containing instructions, scripts, and resources. Cursor automatically detects and uses relevant skills.
...
sequenceDiagram
    ...
    User->>Cursor: "Create Excel report"
    Cursor->>System: Scan available skills
    System->>System: Load skill metadata
    Cursor->>Cursor: Match user request to skills
    Cursor->>Skill: Load xlsx skill SKILL.md
    Skill-->>Cursor: Return instructions + tools
    Cursor->>Cursor: Execute skill
    Cursor->>User: Generate Excel file
```
...
| Personal | `~/.cursor/skills/` | Individual | No | Manual | Personal automation |
| Project | `.cursor/skills/` | Team | Yes | Git | Team standards |
...
Cursor now includes 5 bundled skills available out of the box:
...
| **Cursor API** | `/cursor-api` | Interact with model provider APIs from Cursor |
...
**File:** `~/.cursor/skills/code-review/SKILL.md`
...
Cursor: I'll conduct a comprehensive code review using my Code Review Skill.
...
**File:** `.cursor/skills/brand-voice/SKILL.md`
...
- Use active voice: "Cursor generates reports" not "Reports are generated by Cursor"
...
### ✅ Good Example
"Cursor automates your code review process. Instead of manually checking each PR, Cursor reviews security, performance, and quality—saving your team hours every week."

Why it works: Clear value, specific benefits, action-oriented

### ❌ Bad Example
"Cursor leverages cutting-edge AI to provide comprehensive software development solutions."
...
"Save 8 hours per week on code reviews. Cursor reviews your PRs automatically."

Empathetic support:
"We know deployments can be stressful. Cursor handles testing so you don't have to worry."

Confident product feature:
"Cursor doesn't just suggest code. It understands your architecture and maintains consistency."
...
**File:** `.cursor/skills/doc-generator/SKILL.md`
...
```mermaid
graph TD
    A["User Request"] --> B["Cursor Analyzes"]
...
Cursor Plugins are bundled collections of customizations (slash commands, subagents, MCP servers, and hooks) that install with a single command. They represent the highest-level extension mechanism—combining multiple features into cohesive, shareable packages.
...
sequenceDiagram
    ...
    User->>Cursor: /plugin install pr-review
    Cursor->>Plugin: Download plugin manifest
    Plugin-->>Cursor: Return plugin definition
    Cursor->>Install: Extract components
...
|------|-------|--------|-----------|----------|
| Official | Global | All users | Cursor | PR Review, Security Guidance |
...
2. Write `.cursor-plugin/plugin.json` manifest
...
```mermaid
graph LR
    A["Session Start"] -->|Load| B["Memory (CURSOR.md)"]
...
### 2. Memory Lookup
- Loads CURSOR.md with support standards
...
sequenceDiagram
    participant User
    participant Cursor as Cursor
    participant Memory as Memory<br/>CURSOR.md
    participant MCP as MCP Servers
    participant Skills as Skills
    participant SubAgent as Subagents

    User->>Cursor: Request: "Build auth system"
    Cursor->>Memory: Load project standards
    Memory-->>Cursor: Auth standards, team practices
    Cursor->>MCP: Query GitHub for similar implementations
    MCP-->>Cursor: Code examples, best practices
    Cursor->>Skills: Detect matching Skills
    Skills-->>Cursor: Security Review Skill + Testing Skill
    Cursor->>SubAgent: Delegate implementation
    SubAgent->>SubAgent: Build feature
    Cursor->>Skills: Apply Security Review Skill
    Skills-->>Cursor: Security checklist results
    Cursor->>SubAgent: Delegate testing
    SubAgent-->>Cursor: Test results
    Cursor->>User: Complete system delivered
```
...
- Document team standards in CURSOR.md
...
Hooks are event-driven shell commands that execute automatically in response to Cursor events. They enable automation, validation, and custom workflows without manual intervention.

### Hook Events

Cursor supports **25 hook events** across four hook types (command, http, prompt, agent):

| Hook Event | Trigger | Use Cases |
|------------|---------|-----------|
| **SessionStart** | Session begins/resumes/clear/compact | Environment setup, initialization |
| **InstructionsLoaded** | CURSOR.md or rules file loaded | Validation, transformation, augmentation |
...
| **Stop** | Cursor finishes responding | Summary generation, cleanup tasks |
...
Hooks are configured in `~/.cursor/settings.json` (user-level) or `.cursor/settings.json` (project-level):
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "prettier --write $CURSOR_FILE_PATH"
          }
        ]
      }
    ],
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "eslint $CURSOR_FILE_PATH"
          }
        ]
      }
    ]
...
- `$CURSOR_FILE_PATH` - Path to file being edited/written
- `$CURSOR_TOOL_NAME` - Name of tool being used
- `$CURSOR_SESSION_ID` - Current session identifier
- `$CURSOR_PROJECT_DIR` - Project directory path
...
cursor -p "Should we use microservices or monolith?"
```
...
**Usage:**
```bash
cursor --permission-mode plan          # Read-only analysis
cursor --permission-mode acceptEdits   # Auto-accept edits
cursor --permission-mode auto          # Auto-approve safe actions
cursor --permission-mode dontAsk       # No confirmation prompts
```

### Headless Mode (Print Mode)

Run Cursor without interactive input for automation and CI/CD using the `-p` (print) flag.
...
- name: AI Code Review
  run: cursor -p "Review PR changes and report issues"

# JSON output for scripting
cursor -p --output-format json "list all functions in src/"
```
...
Cursor can integrate with the Chrome browser for web automation tasks. This enables capabilities like navigating web pages, filling forms, taking screenshots, and extracting data from websites directly within your development workflow.
...
cursor -r "Feature"    # Resume session by name/ID
```
...
*Configure models in Cursor Settings*
