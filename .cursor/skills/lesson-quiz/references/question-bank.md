...
### Q1
- **Category**: conceptual
- **Question**: What are the four types of slash commands in Cursor?
...
- **Correct**: A
- **Explanation**: Cursor has built-in commands (like /help, /compact), skills (SKILL.md files), plugin commands (namespaced plugin-name:command), and MCP prompts (/mcp__server__prompt).
...
### Q3
- **Category**: conceptual
- **Question**: When both a skill (.cursor/skills/name/SKILL.md) and a legacy command (.cursor/commands/name.md) exist with the same name, which takes priority?
- **Options**: A) The legacy command | B) The skill | C) Whichever was created first | D) Cursor asks the user to choose
- **Correct**: B
...
- **Correct**: B
- **Explanation**: The `!`command`` syntax runs a shell command and injects its output into the skill prompt before Cursor sees it.
- **Review**: Dynamic context injection section

### Q5
- **Category**: conceptual
- **Question**: What does `disable-model-invocation: true` do in a skill's frontmatter?
- **Options**: A) Prevents the skill from running entirely | B) Allows only the user to invoke it (Cursor cannot auto-invoke) | C) Hides it from the /help menu | D) Disables the skill's AI processing
- **Correct**: B
- **Explanation**: `disable-model-invocation: true` means only the user can trigger the command via `/command-name`. Cursor will never auto-invoke it, useful for skills with side effects like deployments.
- **Review**: Controlling invocation section

### Q6
- **Category**: practical
- **Question**: You want to create a skill that only Cursor can invoke automatically (hidden from the user's / menu). Which frontmatter field do you set?
- **Options**: A) `disable-model-invocation: true` | B) `user-invocable: false` | C) `hidden: true` | D) `auto-only: true`
- **Correct**: B
- **Explanation**: `user-invocable: false` hides the skill from the user's slash menu but allows Cursor to invoke it automatically based on context.
- **Review**: Invocation control matrix

### Q7
- **Category**: practical
- **Question**: What is the correct directory structure for a new custom skill called "deploy"?
- **Options**: A) `.cursor/commands/deploy.md` | B) `.cursor/skills/deploy/SKILL.md` | C) `.cursor/skills/deploy.md` | D) `.cursor/deploy/SKILL.md`
- **Correct**: B
- **Explanation**: Skills live in a directory under `.cursor/skills/` with a `SKILL.md` file inside. The directory name matches the command name.
...
### Q1
- **Category**: conceptual
- **Question**: How many levels does the Cursor memory hierarchy have, and what has the highest priority?
...
- **Correct**: B
- **Explanation**: The `#` prefix pattern allows quick single-rule additions during conversation. Cursor will ask which memory level to save it to.
- **Review**: Quick memory updates section

### Q3
- **Category**: conceptual
- **Question**: What is the maximum depth for `@path/to/file` imports in CURSOR.md?
...
- **Options**: A) Put the rule in `src/api/CURSOR.md` | B) Add `paths: src/api/**` YAML frontmatter to a `.cursor/rules/*.md` file | C) Name the file `.cursor/rules/api.md` | D) Use `@scope: src/api` in the rule file
- **Correct**: B
- **Explanation**: Files in `.cursor/rules/` support a `paths:` frontmatter field with glob patterns to scope rules to specific directories.
...
- **Options**: A) `~/.cursor/CURSOR.md` | B) `CURSOR.local.md` | C) `.cursor/rules/personal.md` | D) `.cursor/memory/personal.md`
- **Correct**: B
- **Explanation**: `CURSOR.local.md` in the project root is for personal project-specific preferences. It should be git-ignored.
- **Review**: Memory locations comparison

### Q7
- **Category**: conceptual
- **Question**: What does the `/init` command do?
- **Options**: A) Initializes a new Cursor project from scratch | B) Generates a template CURSOR.md based on your project structure | C) Resets all memory to defaults | D) Creates a new session
- **Correct**: B
- **Explanation**: `/init` analyzes your project and generates a template CURSOR.md with suggested rules and standards. It's a one-time bootstrapping tool.
- **Review**: /init command section

### Q8
- **Category**: practical
- **Question**: How do you disable Auto Memory completely?
- **Options**: A) Delete the ~/.cursor/projects directory | B) Set `CURSOR_DISABLE_AUTO_MEMORY=1` | C) Add `auto-memory: false` to CURSOR.md | D) Use `/memory disable auto`
- **Correct**: B
- **Explanation**: Setting `CURSOR_DISABLE_AUTO_MEMORY=1` disables auto memory. Value `0` forces it on. Unset = default on.
...
### Q10
- **Category**: practical
- **Question**: You work across two repositories and want Cursor to load CURSOR.md from both. What flag do you use?
- **Options**: A) `--multi-repo` | B) `--add-dir /path/to/other` | C) `--include /path/to/other` | D) `--merge-context /path/to/other`
- **Correct**: B
- **Explanation**: The `--add-dir` flag loads CURSOR.md from additional directories, allowing multi-repo context.
...
### Q2
- **Category**: practical
- **Question**: What is the most important factor for a skill to be auto-invoked by Cursor?
...
- **Correct**: B
- **Explanation**: Cursor decides whether to auto-invoke a skill based solely on its `description` field. It must include specific trigger phrases and scenarios.
...
- **Options**: A) Set `isolation: true` in frontmatter | B) Set `context: fork` with an `agent` field in frontmatter | C) Set `subagent: true` in frontmatter | D) Put the skill in `.cursor/agents/`
- **Correct**: B
...
- **Options**: A) Inline in SKILL.md | B) In a `references/api-spec.md` file inside the skill directory | C) In the project's CURSOR.md | D) In a separate `.cursor/rules/` file
- **Correct**: B
- **Explanation**: Large reference material belongs in the `references/` subdirectory. Cursor loads Level 3 resources on demand, keeping SKILL.md lean.
...
- **Correct**: B
- **Explanation**: Reference Content adds domain knowledge to Cursor's context (e.g., brand guidelines). Task Content provides actionable step-by-step instructions for a workflow.
...
- **Correct**: B
- **Explanation**: The name must be kebab-case (lowercase, hyphens), max 64 characters, and cannot contain reserved words such as `"cursor"` (see Cursor docs).
- **Review**: SKILL.md format section

### Q9
- **Category**: conceptual
- **Question**: In what order does Cursor search for skills?
...
### Q10
- **Category**: practical
- **Question**: How do you prevent Cursor from automatically invoking a skill while still allowing users to use it manually?
...
- **Correct**: B
- **Explanation**: `disable-model-invocation: true` prevents Cursor from auto-invoking but keeps the skill available in the user's `/` menu for manual use.
- **Review**: Controlling invocation section
...
- **Correct**: B
- **Explanation**: CLI-defined agents (`--agents` flag) override User-level (`~/.cursor/agents/`), which override Project-level (`.cursor/agents/`).
...
- **Options**: A) Gives read access to the project CURSOR.md | B) Creates a persistent memory directory scoped to the current project | C) Shares the main agent's conversation history | D) Loads the project's git history
- **Correct**: B
...
### Q8
- **Category**: practical
- **Question**: How do you include a phrase in a subagent's description to encourage Cursor to automatically delegate tasks to it?
...
- **Correct**: B
- **Explanation**: Including phrases like "use PROACTIVELY" or "MUST BE USED" in the description strongly encourages Cursor to automatically delegate matching tasks.
...
- **Options**: A) Use `/resume agent-id` | B) Pass the `resume` parameter with the agentId when calling Task tool | C) Use `cursor -r agent-id` | D) Subagents cannot be resumed
- **Correct**: B
...
- **Options**: A) `cursor mcp install github` | B) `cursor mcp add --transport http github https://api.github.com/mcp` | C) `cursor plugin add github-mcp` | D) `cursor connect github`
- **Correct**: B
- **Explanation**: Use `cursor mcp add` with `--transport` flag, a name, and the server URL. For stdio: `cursor mcp add github -- npx -y @modelcontextprotocol/server-github`.
...
- **Options**: A) They are truncated | B) Tool Search auto-enables to dynamically select relevant tools | C) Cursor shows an error | D) Extra tools are disabled
- **Correct**: B
...
- **Options**: A) It loads automatically | B) They get an approval prompt to trust the project's MCP servers | C) It's ignored unless they opt in via settings | D) Cursor asks the admin to approve
- **Correct**: B
...
### Q7
- **Category**: conceptual
- **Question**: What does `cursor mcp serve` do?
- **Options**: A) Starts an MCP server dashboard | B) Makes Cursor itself act as an MCP server for other applications | C) Serves MCP documentation | D) Tests MCP server connections
- **Correct**: B
- **Explanation**: `cursor mcp serve` turns Cursor into an MCP server, enabling multi-agent orchestration where one Cursor instance can be controlled by another.
- **Review**: Cursor as MCP Server section
...
### Q1
- **Category**: conceptual
- **Question**: What are the four types of hooks in Cursor?
...
- **Options**: A) Non-blocking warning shown | B) Blocking error — stderr is shown as an error to Cursor, tool use is prevented | C) Hook is retried | D) Session ends
- **Correct**: B
...
- **Correct**: B
- **Explanation**: A PreToolUse hook can output JSON with `"updatedInput": {...}` on stdout (with exit 0) to modify the tool's parameters before Cursor uses them.
- **Review**: PreToolUse output section

### Q5
- **Category**: conceptual
- **Question**: Which hook event supports `CURSOR_ENV_FILE` for persisting environment variables into the session?
- **Options**: A) PreToolUse | B) UserPromptSubmit | C) SessionStart | D) All events
- **Correct**: C
- **Explanation**: Only SessionStart hooks can use `CURSOR_ENV_FILE` to persist environment variables into the session.
...
### Q9
- **Category**: conceptual
- **Question**: How many hook events does Cursor support in total?
- **Options**: A) 10 | B) 16 | C) 25 | D) 30
- **Correct**: C
- **Explanation**: Cursor supports 25 hook events: PreToolUse, PostToolUse, PostToolUseFailure, UserPromptSubmit, Stop, StopFailure, SubagentStop, SubagentStart, PermissionRequest, Notification, PreCompact, PostCompact, SessionStart, SessionEnd, WorktreeCreate, WorktreeRemove, ConfigChange, CwdChanged, FileChanged, TeammateIdle, TaskCompleted, TaskCreated, Elicitation, ElicitationResult, InstructionsLoaded.
...
- **Options**: A) `plugin.yaml` in the root directory | B) `.cursor-plugin/plugin.json` | C) `package.json` with a "cursor" key | D) `.cursor/plugin.md`
- **Correct**: B
- **Explanation**: The plugin manifest lives at `.cursor-plugin/plugin.json` with required fields: name, description, version, author.
- **Review**: Plugin definition structure section

### Q2
- **Category**: practical
- **Question**: How do you test a plugin locally before publishing?
- **Options**: A) Use `/plugin test ./my-plugin` | B) Use `cursor --plugin-dir ./my-plugin` | C) Use `cursor plugin validate ./my-plugin` | D) Copy it to ~/.cursor/plugins/
- **Correct**: B
...
- **Options**: A) `$PLUGIN_HOME` | B) `${CURSOR_PLUGIN_ROOT}` | C) `$PLUGIN_DIR` | D) `${CURSOR_PLUGIN_PATH}`
- **Correct**: B
- **Explanation**: `${CURSOR_PLUGIN_ROOT}` resolves to the plugin's installed directory, enabling portable path references in hooks and MCP configs.
...
- **Options**: A) `cursor plugin add github:username/repo` | B) `/plugin install github:username/repo` | C) `npm install @cursor/username-repo` | D) `git clone` then `cursor plugin register`
- **Correct**: B
...
- **Options**: A) Edit a config file manually | B) Use `/plugin enable`, `/plugin disable`, `/plugin update plugin-name` | C) Use `cursor plugin-manager` | D) Reinstall the plugin
- **Correct**: B
- **Explanation**: Cursor provides slash commands for full lifecycle management: enable, disable, update, uninstall.
- **Review**: Installation methods section
...
- **Options**: A) `.cursor-plugin/hooks.json` | B) `hooks/hooks.json` | C) `plugin.json` hooks section | D) `.cursor/settings.json`
- **Correct**: B
...
- **Correct**: B
- **Explanation**: Checkpoints capture conversation messages, file modifications made by Cursor's tools, tool usage history, and session context.
- **Review**: Overview section
...
### Q4
- **Category**: practical
- **Question**: You used `rm -rf temp/` via Bash in Cursor, then want to rewind. Will the checkpoint restore those files?
...
- **Correct**: B
- **Explanation**: Checkpoints only track file changes made by Cursor's tools (Write, Edit). Bash commands like rm, mv, cp operate outside checkpoint tracking.
...
- **Correct**: B
- **Explanation**: Automatic checkpoints are created with every user prompt, capturing the state before Cursor processes the request.
...
### Q1
- **Category**: conceptual
- **Question**: What are the six permission modes in Cursor?
...
### Q6
- **Category**: practical
- **Question**: How do you run Cursor in a CI/CD pipeline with structured JSON output and a turn limit?
- **Options**: A) `cursor --ci --json --limit 3` | B) `cursor -p --output-format json --max-turns 3 "review code"` | C) `cursor --pipeline --format json` | D) `cursor run --json --turns 3`
- **Correct**: B
...
- **Options**: A) A list of running background processes | B) A persistent to-do list that survives context compaction, shareable via `CURSOR_TASK_LIST_ID` | C) A history of past sessions | D) A queue of pending tool calls
- **Correct**: B
- **Explanation**: The Task List (Ctrl+T) is persistent across context compactions and can be shared across sessions via named task directories using `CURSOR_TASK_LIST_ID`.
...
### Q1
- **Category**: conceptual
- **Question**: What are the two primary modes of the Cursor CLI?
- **Options**: A) Online and offline mode | B) Interactive REPL (`cursor`) and Print mode (`cursor -p`) | C) GUI and terminal mode | D) Single and batch mode
- **Correct**: B
...
### Q2
- **Category**: practical
- **Question**: How do you pipe a file into Cursor and get JSON output?
- **Options**: A) `cursor --file error.log --json` | B) `cat error.log | cursor -p --output-format json "explain this"` | C) `cursor < error.log --format json` | D) `cursor -p --input error.log --json`
- **Correct**: B
...
### Q4
- **Category**: practical
- **Question**: How do you guarantee schema-valid JSON output from Cursor?
...
### Q6
- **Category**: practical
- **Question**: How do you restrict Cursor to only use read-only tools for a security audit?
- **Options**: A) `cursor --read-only "audit code"` | B) `cursor --permission-mode plan --tools "Read,Grep,Glob" "audit code"` | C) `cursor --safe-mode "audit code"` | D) `cursor --no-write "audit code"`
- **Correct**: B
- **Explanation**: Combine `--permission-mode plan` (read-only analysis) with `--tools` (allowlist of specific tools) to restrict Cursor to only read operations.
...
- **Correct**: B
- **Explanation**: CLI-defined agents (--agents flag) have highest priority, then User-level (~/.cursor/agents/), then Project-level (.cursor/agents/).
...
### Q9
- **Category**: conceptual
- **Question**: What exit code does `cursor auth status` return when the user is logged in?
- **Options**: A) 1 | B) 0 | C) 200 | D) It doesn't return an exit code
- **Correct**: B
- **Explanation**: `cursor auth status` exits with code 0 when logged in, 1 when not. This makes it scriptable for CI/CD authentication checks.
- **Review**: CLI commands table

### Q10
- **Category**: practical
- **Question**: How do you process multiple files in a batch with Cursor?
- **Options**: A) `cursor --batch *.md` | B) Use a for loop: `for file in *.md; do cursor -p "summarize: $(cat $file)" > ${file%.md}.json; done` | C) `cursor -p --files *.md "summarize all"` | D) Batch processing is not supported
- **Correct**: B
