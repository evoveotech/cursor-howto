...
version: 2.2.0
description: Comprehensive Cursor self-assessment and learning path advisor. Runs a multi-category quiz covering 10 feature areas, produces a detailed skill profile with per-topic scores, identifies specific gaps, and generates a personalized learning path with prioritized next steps. Use when asked to "assess my level", "take the quiz", "find my level", "where should I start", "what should I learn next", "check my skills", "skill check", or "level up".
---

# Self-Assessment & Learning Path Advisor

Comprehensive interactive assessment that evaluates Cursor proficiency across 10 feature areas, identifies specific skill gaps, and generates a personalized learning path to level up.
...
"Part 1/2: Which of these Cursor skills do you already have?"
Options:
1. "Start Cursor and chat" — I can run `cursor` and interact with it
2. "Created/edited CURSOR.md" — I have set up project or user memory
...
2. "Set up hooks" — Configured hooks in ~/.cursor/settings.json
3. "Created/used subagents" — Used .cursor/agents/ for task delegation
4. "Used print mode (cursor -p)" — Used `cursor -p` for non-interactive or CI/CD use
...
Options:
1. "Created a custom slash command or skill" — Written a SKILL.md file with frontmatter, or created .cursor/commands/ files
2. "Used dynamic context in commands" — Used `$ARGUMENTS`, `$0`/`$1`, backtick `!command` syntax, or `@file` references in skill/command files
3. "Set up project + personal memory" — Created both a project CURSOR.md and personal ~/.cursor/CURSOR.md (or CURSOR.local.md)
4. "Used memory hierarchy features" — Understand the 7-level priority order, used .cursor/rules/ directory, path-specific rules, or @import syntax
...
2. "Used advanced MCP features" — Project-scope .mcp.json, OAuth authentication, MCP resources with @mentions, Tool Search, or `cursor mcp serve`
3. "Created or configured custom subagents" — Defined agents in .cursor/agents/ with custom tools, model, or permissions
...
4. "Used remote/desktop/web features" — Used `cursor remote-control`, `cursor --remote`, `/teleport`, `/desktop`, or worktrees with `cursor -w`
...
"Which of these have you done? Select all that apply."
Options:
1. "Installed or created a plugin" — Used a bundled plugin from marketplace, or created a .cursor-plugin/ directory with plugin.json manifest
...
3. "Used print mode in scripts or CI/CD" — Used `cursor -p` with --output-format json, --max-turns, piped input, or integrated into GitHub Actions / CI pipelines
...
```markdown
## Cursor Skill Assessment Results
...
- Focus on: CURSOR.md creation, `/init` and `/memory` commands, `#` prefix for quick updates
- Key exercise: Create a project CURSOR.md with your coding standards
- Done when: Cursor remembers your preferences across sessions

**Memory (score 1 — review)**:
- Focus on: 7-level hierarchy and priority order, .cursor/rules/ directory with path-specific rules, `@import` syntax (max depth 5), Auto Memory MEMORY.md (200-line limit)
...
- Focus on: Remote control (`cursor remote-control`), web sessions (`cursor --remote`), desktop handoff (`/desktop`), worktrees (`cursor -w`), task lists (Ctrl+T), managed settings for enterprise
- Done when: You can hand off sessions between CLI, web, and desktop

**Plugins (score 0)**:
- Tutorial: [07-plugins/](../../../07-plugins/)
- Focus on: Plugin structure (.cursor-plugin/plugin.json), what plugins bundle (commands, agents, MCP, hooks, settings), installation from marketplace
...
- Focus on: Creating plugin.json manifest, plugin hooks (hooks/hooks.json), LSP configuration (.lsp.json), `${CURSOR_PLUGIN_ROOT}` variable, --plugin-dir for testing, marketplace publishing
- Done when: You can create and test a plugin for your team

**CLI (score 0)**:
- Tutorial: [10-cli/](../../../10-cli/)
- Focus on: Interactive vs print mode, `cursor -p` with piping, `--output-format json`, session management (-c/-r)
- Key exercise: Pipe a file to `cursor -p` and get JSON output
- Done when: You can use Cursor non-interactively in a script

**CLI (score 1 — review)**:
- Focus on: --agents flag with JSON config, --json-schema for structured output, --fallback-model, --from-pr, --strict-mcp-config, batch processing with for loops, `cursor mcp serve`
- Done when: You have a CI/CD script that uses Cursor with structured JSON output
...
- "skill check"
- "level up"
- "how good am I at Cursor"
- "evaluate my Cursor knowledge"
