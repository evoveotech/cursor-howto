...
# Cursor Examples - Complete Index
...
**Installation Path**: `.cursor/commands/`
...
|------|-------------|-------|----------|
| `project-CURSOR.md` | Team project standards | Project-wide | `./CURSOR.md` |
| `directory-api-CURSOR.md` | API-specific rules | Directory | `./src/api/CURSOR.md` |
| `personal-CURSOR.md` | Personal preferences | User | `~/.cursor/CURSOR.md` |
| `memory-saved.png` | Screenshot: memory saved | - | Visual reference |
| `memory-ask-cursor.png` | Screenshot: ask Cursor | - | Visual reference |
...
### Cursor MD Skill (1 file)
```
cursor-md/
└── SKILL.md                          # Skill definition
```

**Purpose**: Manage and optimize CURSOR.md files
...
**Installation Path**: `~/.cursor/skills/` or `.cursor/skills/`
...
**Installation Path**: `.cursor/agents/`
...
**Installation Path**: `.mcp.json` (project scope) or `~/.cursor/mcp.json` (user scope)
...
**Installation Path**: Configure in `~/.cursor/settings.json`
...
**Key CLI Features**:
- `cursor` - Start interactive session
- `cursor -p "prompt"` - Headless/non-interactive mode
- `cursor web` - Launch web session
- `cursor --model` - Select model (Sonnet 4.6, Opus 4.6)
- `cursor --permission-mode` - Set permission mode
- `cursor --remote` - Enable remote control via WebSocket
...
|------|----------|-------------|
| `CURSOR_SCOPE.md` | `/docs/` | Cursor IDE scope: how this repo maps to Rules, AGENTS.md, MCP |
...
├── INDEX.md                                     # This file
├── QUICK_REFERENCE.md                           # Quick reference card
├── cursor_concepts_guide.md                     # Original guide
│
...
# Configure hooks in settings (~/.cursor/settings.json)
...
- Examples follow Cursor best practices
