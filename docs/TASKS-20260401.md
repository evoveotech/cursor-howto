# Tasks: Dual-Layer Knowledge Base — cursor-howto
...
|---|-----------|--------|-------------|
| M1 | Infrastructure Live | End of April 2026 | quickstart.sh, CURSOR.md, staleness action, agent index generator live; 2 weakest modules upgraded |
...
- [ ] **T-001** — Write `scripts/quickstart.sh`
  - Detects `~/.cursor/` directory (creates with user confirmation if missing)
  - Copies first slash command + CURSOR.md + skill to user's setup
  - Appends agent-discovery line to CURSOR.md linking to AGENT-INDEX.md
...
- [ ] **T-005** — Add version badges to all 10 module READMEs
  - Format: `> ✅ Verified against Cursor **vX.X.X** · Last verified: YYYY-MM-DD`
...
### Pillar 6: Content Quality (CURSOR.md)

- [ ] **T-007** — Create root `CURSOR.md` for the project
...
- [ ] **T-056** — Create `skills/cursor-howto-lookup/SKILL.md`
  - Reads `agent-manifest.json` to find relevant modules
  - Uses `Read` to load the section
  - Installation: `cp -r cursor-howto/skills/cursor-howto-lookup ~/.cursor/skills/`
...
- [ ] **T-063** — Open pinned GitHub Discussion: "Share your Cursor workflows"
...
- [ ] **T-064** — Run `/docs-sync-cursor-code` skill after every CC release; update `WHATS-NEW.md`
