...
# Cursor Examples - Quick Reference Card

*Evoveo Tech (**EVOVEO INNOVATIONS LIMITED**) — Cursor IDE & Cursor CLI patterns.*

**Cursor IDE:** Paths below are **templates** from this repo. In real projects, prefer **`.cursor/rules/`** and **`AGENTS.md`** for always-on instructions; use these files as **saved prompts** or paste into Chat/Agent. See [docs/CURSOR_SCOPE.md](docs/CURSOR_SCOPE.md).
...
# Install specific
cp 04-subagents/code-reviewer.md .cursor/agents/
```
...
# Or user scope: add to ~/.cursor/mcp.json
```
...
# Configure in settings (~/.cursor/settings.json)
```
...
cursor -r "session"    # Resume session by name/ID
```
...
|---------|-------------|-------|
| **Slash Commands (55+)** | `.cursor/commands/*.md` | `/command-name` |
| **Memory** | `./CURSOR.md` | Auto-loaded |
| **Skills** | `.cursor/skills/*/SKILL.md` | Auto-invoked |
| **Subagents** | `.cursor/agents/*.md` | Auto-delegated |
| **MCP** | `.mcp.json` (project) or `~/.cursor/mcp.json` (user) | `/mcp__server__action` |
| **Hooks (25 events)** | `~/.cursor/hooks/*.sh` | Event-triggered (4 types) |
...
| **Web Sessions** | Built-in | `cursor web` |
| **Git Worktrees** | Built-in | `/worktree` |
| **Auto Memory** | Built-in | Auto-saves to CURSOR.md |
| **Task List** | Built-in | `/task list` |
| **Bundled Skills (5)** | Built-in | `/simplify`, `/loop`, `/cursor-api`, `/voice`, `/browse` |
...
```bash
# Slash command
cp 01-slash-commands/generate-api-docs.md .cursor/commands/

# Subagent
cp 04-subagents/documentation-writer.md .cursor/agents/

# Skill
cp -r 03-skills/doc-generator ~/.cursor/skills/
...
cursor --enable-auto-mode -p "Refactor and test the auth module"
...
# Review and approve
# Cursor implements systematically
```
...
cursor -p "Run tests" --permission-mode dontAsk

# With Auto Mode for fully autonomous CI tasks
cursor --enable-auto-mode -p "Run tests and fix failures"
...
└── src/
    └── api/
        └── CURSOR.md          # Directory-specific memory
...
│   └── CURSOR.md              # Personal memory
└── .cursor/mcp.json               # Personal MCP config (user scope)
```
...
| **Voice Dictation** | Speak commands and context to Cursor | `/voice` command |
...
| **Plugin LSP** | Language Server Protocol support for plugins | `userConfig`, `${CURSOR_PLUGIN_DATA}` variable |
| **Remote Control** | Control Cursor via WebSocket API | `cursor --remote` for external integrations |
| **Web Sessions** | Browser-based Cursor interface | `cursor web` to launch |
| **Desktop App** | Native desktop application | Download from cursor.com/download |
| **Task List** | Manage background tasks | `/task list`, `/task status <id>` |
| **Auto Memory** | Automatic memory saving from conversations | Cursor auto-saves key context to CURSOR.md |
...
| **Agent Teams** | Coordinate multiple agents on tasks | Enable with `CURSOR_AGENT_TEAMS=1` env var |
...
# Verify YAML syntax
head -20 .cursor/agents/code-reviewer.md
...
| Team standards | Memory | `02-memory/project-CURSOR.md` |
...
- **Original Guide**: `cursor_concepts_guide.md`
...
A: Commit `.cursor/` directory to git.
