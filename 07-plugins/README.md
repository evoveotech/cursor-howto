...
This folder contains complete plugin examples that bundle multiple Cursor features into cohesive, installable packages.
...
Plugin manifest uses JSON format in `.cursor-plugin/plugin.json`:
...
## Persistent Plugin Data (`${CURSOR_PLUGIN_DATA}`) (v2.1.78+)

Plugins have access to a persistent state directory via the `${CURSOR_PLUGIN_DATA}` environment variable. This directory is unique per plugin and survives across sessions, making it suitable for caches, databases, and other persistent state:
...
```json
{
  "hooks": {
    "PostToolUse": [
      {
        "command": "node ${CURSOR_PLUGIN_DATA}/track-usage.js"
      }
    ]
  }
}
...
|----------|---------------|---|---|
| **Standalone** | `/hello` | Manual setup in CURSOR.md | Personal, project-specific |
...
The official Cursor plugin directory is `cursor/cursor-plugins-official`. Enterprise admins can also create private plugin marketplaces for internal distribution.

```mermaid
graph TB
    A["Plugin Marketplace"]
    B["Official<br/>cursor/cursor-plugins-official"]
...
Plugin marketplaces are defined in `.cursor-plugin/marketplace.json`:
...
| **npm** | `{ "source": "npm", "package": "..." }` | `{ "source": "npm", "package": "@acme/cursor-plugin", "version": "^2.0" }` |
| **pip** | `{ "source": "pip", "package": "..." }` | `{ "source": "pip", "package": "cursor-data-plugin", "version": ">=1.0" }` |
...
**Official marketplace submission**: Submit plugins to the Cursor marketplace for broader distribution.
...
```bash
cursor plugin install <name>@<marketplace>   # Install from a marketplace
cursor plugin uninstall <name>               # Remove a plugin
cursor plugin list                           # List installed plugins
cursor plugin enable <name>                  # Enable a disabled plugin
cursor plugin disable <name>                 # Disable a plugin
cursor plugin validate                       # Validate plugin structure
```
...
# or from CLI:
cursor plugin install plugin-name@marketplace-name
```
...
cursor --plugin-dir ./path/to/plugin
cursor --plugin-dir ./plugin-a --plugin-dir ./plugin-b
```
...
```bash
cursor --plugin-dir ./my-plugin
cursor --plugin-dir ./my-plugin --plugin-dir ./another-plugin
```

This launches Cursor with your plugin loaded, allowing you to:
...
Plugins support hot-reload during development. When you modify plugin files, Cursor can detect changes automatically. You can also force a reload with:
...
4. Test locally with `cursor --plugin-dir ./my-plugin`
...
The following Cursor features work together with plugins:
...
- Check Cursor version compatibility: `/version`
...
- Restart Cursor: `exit` and reopen
...
- [Official Plugins Documentation](https://docs.cursor.com/docs/en/plugins)
- [Discover Plugins](https://docs.cursor.com/docs/en/discover-plugins)
- [Plugin Marketplaces](https://docs.cursor.com/docs/en/plugin-marketplaces)
- [Plugins Reference](https://docs.cursor.com/docs/en/plugins-reference)
