...
> Conventions and formatting rules for contributing to this **Evoveo Tech** guide (Cursor IDE & Cursor CLI). Follow this guide to keep content consistent, professional, and easy to maintain.
...
| **Memory file** | Scope-prefixed | `project-CURSOR.md`, `personal-CURSOR.md` |
| **Top-level docs** | UPPER_CASE `.md` | `CATALOG.md`, `QUICK_REFERENCE.md`, `CONTRIBUTING.md` |
| **Image assets** | Kebab-case | `pr-slash-command.png`, `cursor-howto-logo.svg` |
...
| `Code` (`` `text` ``) | File names, commands, config values, code references | `` `CURSOR.md` `` |
...
cursor mcp add notion --transport http https://mcp.notion.com/mcp
```
...
```markdown
[Cursor's official documentation](https://docs.cursor.com/docs/en/overview)
```
...
- **Beginner-friendly** — assume the reader is new to Cursor, not new to programming
...
- Use "Cursor" (not "Cursor CLI" or "the tool")
