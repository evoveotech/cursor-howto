...
This folder contains comprehensive documentation and examples for MCP server configurations and usage with Cursor.
...
Cursor supports multiple transport protocols for MCP server connections:

### HTTP Transport (Recommended)

```bash
# Basic HTTP connection
cursor mcp add --transport http notion https://mcp.notion.com/mcp

# HTTP with authentication header
cursor mcp add --transport http secure-api https://api.example.com/mcp \
...
cursor mcp add --transport stdio myserver -- npx @myorg/mcp-server

# With environment variables
cursor mcp add --transport stdio myserver --env KEY=value -- npx server
```
...
```bash
cursor mcp add --transport sse legacy-server https://example.com/sse
```
...
```bash
cursor mcp add --transport ws realtime-server wss://example.com/mcp
```
...
```bash
cursor mcp add --transport stdio my-server -- cmd /c npx -y @some/package
```

### OAuth 2.0 Authentication

Cursor supports OAuth 2.0 for MCP servers that require it. When connecting to an OAuth-enabled server, Cursor handles the entire authentication flow:

```bash
# Connect to an OAuth-enabled MCP server (interactive flow)
cursor mcp add --transport http my-service https://my-service.example.com/mcp
...
If your MCP server returns errors on the standard OAuth metadata endpoint (`/.well-known/oauth-authorization-server`) but exposes a working OIDC endpoint, you can tell Cursor to fetch OAuth metadata from a specific URL. Set `authServerMetadataUrl` in the `oauth` object of your server config:
...
The URL must use `https://`. This option requires Cursor v2.1.64 or later.

### Cursor.ai MCP Connectors

MCP servers configured in your Cursor.ai account are automatically available in Cursor. This means any MCP connections you set up through the Cursor.ai web interface will be accessible without additional configuration.

Cursor.ai MCP connectors are also available in `--print` mode (v2.1.83+), enabling non-interactive and scripted usage.

To disable Cursor.ai MCP servers in Cursor, set the `ENABLE_CURSOR_MCP_SERVERS` environment variable to `false`:

```bash
ENABLE_CURSOR_MCP_SERVERS=false cursor
```

> **Note:** This feature is only available for users logged in with Cursor.ai accounts.
...
When MCP tool descriptions exceed 10% of the context window, Cursor automatically enables tool search to efficiently select the right tools without overwhelming the model context.
...
Cursor supports MCP `list_changed` notifications. When an MCP server dynamically adds, removes, or modifies its available tools, Cursor receives the update and adjusts its tool list automatically -- no reconnection or restart required.
...
As of v2.1.84, Cursor enforces a **2 KB cap** on tool descriptions and instructions per MCP server. This prevents individual servers from consuming excessive context with overly verbose tool definitions, reducing context bloat and keeping interactions efficient.

## MCP Prompts as Slash Commands

MCP servers can expose prompts that appear as slash commands in Cursor. Prompts are accessible using the naming convention:
...
This allows Cursor to fetch and include MCP resource content inline as part of the conversation context.
...
|-------|----------|-------------|-------------|------------------|
| **Local** (default) | `~/.cursor/mcp.json` (under project path) | Private to current user, current project only (was called `project` in older versions) | Just you | No |
| **Project** | `.mcp.json` | Checked into git repository | Team members | Yes (first use) |
| **User** | `~/.cursor/mcp.json` | Available across all projects (was called `global` in older versions) | Just you | No |
...
cursor mcp add --transport http github https://api.github.com/mcp

# Add local stdio server
cursor mcp add --transport stdio database -- npx @company/db-server

# List all MCP servers
cursor mcp list

# Get details on specific server
cursor mcp get github

# Remove an MCP server
cursor mcp remove github

# Reset project-specific approval choices
cursor mcp reset-project-choices

# Import from Cursor Desktop
cursor mcp add-from-cursor-desktop
```
...
cursor mcp add --transport stdio github -- npx @modelcontextprotocol/server-github
```
...
# Or use the CLI to add directly:
cursor mcp add --transport stdio database -- npx @modelcontextprotocol/server-database
```
...
cursor mcp add --transport stdio filesystem -- npx @modelcontextprotocol/server-filesystem /home/user/projects
```
...
## Cursor as MCP Server (`cursor mcp serve`)

Cursor itself can act as an MCP server for other applications. This enables external tools, editors, and automation systems to leverage Cursor's capabilities through the standard MCP protocol.

```bash
# Start Cursor as an MCP server on stdio
cursor mcp serve
```

Other applications can then connect to this server as they would any stdio-based MCP server. For example, to add Cursor as an MCP server in another Cursor instance:

```bash
cursor mcp add --transport stdio cursor-agent -- cursor mcp serve
```

This is useful for building multi-agent workflows where one Cursor instance orchestrates another.
...
**Location:**
- macOS: `/Library/Application Support/Cursor/managed-mcp.json`
- Linux: `~/.config/Cursor/managed-mcp.json`
- Windows: `%APPDATA%\Cursor\managed-mcp.json`
...
Use the `${CURSOR_PLUGIN_ROOT}` variable to reference paths relative to the plugin's installation directory:
...
```json
{
  "mcpServers": {
    "plugin-tools": {
      ...
      "env": {
        "CONFIG_PATH": "${CURSOR_PLUGIN_ROOT}/config.json"
      }
    }
  }
}
...
Cursor enforces limits on MCP tool output to prevent context overflow:
...
As MCP adoption scales, connecting to dozens of servers with hundreds or thousands of tools creates a significant challenge: **context bloat**. This is arguably the biggest problem with MCP at scale, and the MCP community has proposed an elegant solution — using code execution instead of direct tool calls.

> **Source**: [Code Execution with MCP: Building More Efficient Agents](https://www.anthropic.com/engineering/code-execution-with-mcp) — original MCP engineering article
...
| Feature | Description |
|---------|-------------|
| **Zero-config discovery** | Auto-discovers MCP servers from Cursor, Cursor, Codex, or local configs |
...
### MCP Server Crashes
- Check MCP server logs: `~/.cursor/logs/`
...
- [Official MCP Documentation](https://docs.cursor.com/docs/en/mcp)
...
- [Code Execution with MCP](https://www.anthropic.com/engineering/code-execution-with-mcp) — engineering article on solving context bloat
- [Cursor CLI Reference](https://docs.cursor.com/docs/en/cli-reference)
