...
Build an EPUB ebook from the Cursor How-To markdown files.
...
options:
  ...
  --output, -o OUTPUT   Output path (default: cursor-howto-guide.epub)
...
uv run scripts/build_epub.py --output ~/Desktop/cursor-guide.epub
...
**Missing logo**: The script generates a text-only cover if `cursor-howto-logo.png` is not found.
