...
class TestEPUBConfig:
    ...
    def test_default_values(self, tmp_path: Path) -> None:
        ...
        assert config.identifier == "cursor-howto-guide"
        assert config.title == "Cursor How-To Guide"
        assert config.language == "en"
        assert config.author == "EVOVEO INNOVATIONS LIMITED"
...
class TestChapterCollector:
    ...
    def test_path_mapping(self, tmp_project: Path, state: BuildState) -> None:
        ...
        keys = {k.replace("\\", "/") for k in state.path_to_chapter}
        assert "README.md" in keys
        assert "01-test-chapter" in keys
        assert "01-test-chapter/README.md" in keys
