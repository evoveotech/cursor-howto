...
Hooks are automated scripts that execute in response to specific events during Cursor sessions. They enable automation, validation, permission management, and custom workflows.

## Overview

Hooks are automated actions (shell commands, HTTP webhooks, LLM prompts, or subagent evaluations) that execute automatically when specific events occur in Cursor. They receive JSON input and communicate results via exit codes and JSON output.
...
- `~/.cursor/settings.json` - User settings (all projects)
- `.cursor/settings.json` - Project settings (shareable, committed)
- `.cursor/settings.local.json` - Local project settings (not committed)
...
| `command` | Shell command to execute | `"$CURSOR_PROJECT_DIR/.cursor/hooks/format.sh"` |
...
Cursor supports four hook types:
...
{
  "type": "command",
  "command": "python3 \"$CURSOR_PROJECT_DIR/.cursor/hooks/validate.py\"",
  "timeout": 60
}
```
...
LLM-evaluated prompts where the hook content is a prompt that Cursor evaluates. Primarily used with `Stop` and `SubagentStop` events for intelligent task completion checking.

```json
{
  "type": "prompt",
  "prompt": "Evaluate if Cursor completed all requested tasks.",
  "timeout": 30
}
```
...
Cursor supports **25 hook events**:
...
| **InstructionsLoaded** | After CURSOR.md or rules file loaded | (none) | No | Modify/filter instructions |
...
| **Stop** | Cursor finishes responding | (none) | Yes | Task completion check |
...
Runs after Cursor creates tool parameters and before processing. Use this to validate or modify tool inputs.
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$CURSOR_PROJECT_DIR/.cursor/hooks/validate-bash.py"
          }
        ]
      }
    ]
...
Runs immediately after tool completion. Use for verification, logging, or providing context back to Cursor.
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$CURSOR_PROJECT_DIR/.cursor/hooks/security-scan.py"
          }
        ]
      }
    ]
...
**Output control:**
- `"block"` decision prompts Cursor with feedback
- `additionalContext`: Context added for Cursor

### UserPromptSubmit

Runs when user submits a prompt, before Cursor processes it.
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "$CURSOR_PROJECT_DIR/.cursor/hooks/validate-prompt.py"
          }
        ]
      }
    ]
...
Run when Cursor finishes responding (Stop) or a subagent completes (SubagentStop). Supports prompt-based evaluation for intelligent task completion checking.

**Additional input field:** Both `Stop` and `SubagentStop` hooks receive a `last_assistant_message` field in their JSON input, containing the final message from Cursor or the subagent before stopping. This is useful for evaluating task completion.
    ...
    "SubagentStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "$CURSOR_PROJECT_DIR/.cursor/hooks/subagent-init.sh"
          }
        ]
      }
    ]
...
**Special feature:** Use `CURSOR_ENV_FILE` to persist environment variables (also available in `CwdChanged` and `FileChanged` hooks):

```bash
#!/bin/bash
if [ -n "$CURSOR_ENV_FILE" ]; then
  echo 'export NODE_ENV=development' >> "$CURSOR_ENV_FILE"
fi
exit 0
```
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "\"$CURSOR_PROJECT_DIR/.cursor/hooks/session-cleanup.sh\""
          }
        ]
      }
    ]
...
|----------|-------------|-------------|
| `CURSOR_PROJECT_DIR` | All hooks | Absolute path to project root |
| `CURSOR_ENV_FILE` | SessionStart, CwdChanged, FileChanged | File path for persisting env vars |
| `CURSOR_REMOTE` | All hooks | `"true"` if running in remote environments |
| `${CURSOR_PLUGIN_ROOT}` | Plugin hooks | Path to plugin directory |
| `${CURSOR_PLUGIN_DATA}` | Plugin hooks | Path to plugin data directory |
| `CURSOR_SESSIONEND_HOOKS_TIMEOUT_MS` | SessionEnd hooks | Configurable timeout in milliseconds for SessionEnd hooks (overrides default) |
...
**File:** `.cursor/hooks/validate-bash.py`
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python3 \"$CURSOR_PROJECT_DIR/.cursor/hooks/validate-bash.py\""
          }
        ]
      }
    ]
...
**File:** `.cursor/hooks/security-scan.py`
...
**File:** `.cursor/hooks/format-code.sh`
...
**File:** `.cursor/hooks/validate-prompt.py`
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "prompt",
            "prompt": "Review if Cursor completed all requested tasks. Check: 1) Were all files created/modified? 2) Were there unresolved errors? If incomplete, explain what's missing.",
            "timeout": 30
          }
        ]
      }
...
**File:** `.cursor/hooks/context-tracker.py`
...
# Configuration
CONTEXT_LIMIT = 128000  # Cursor's context window (adjust for your model)
...
def get_state_file(session_id: str) -> str:
    ...
    return os.path.join(tempfile.gettempdir(), f"cursor-context-{session_id}.json")
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "python3 \"$CURSOR_PROJECT_DIR/.cursor/hooks/context-tracker.py\""
          }
        ]
      }
    ],
...
2. `Stop` fires after Cursor responds - calculates delta and reports usage
...
> **Note:** There is no single official offline tokenizer for all providers. Both methods are approximations. The transcript includes user prompts, Cursor's responses, and tool outputs, but NOT system prompts or internal context.

### Example 7: Seed Auto-Mode Permissions (One-Time Setup Script)

A one-time setup script that seeds `~/.cursor/settings.json` with ~67 safe permission rules equivalent to Cursor's auto-mode baseline — without any hook, without remembering future choices. Run it once; safe to re-run (skips rules already present).
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "${CURSOR_PLUGIN_ROOT}/scripts/validate.sh"
          }
        ]
      }
    ]
...
- `${CURSOR_PLUGIN_ROOT}` - Path to the plugin directory
- `${CURSOR_PLUGIN_DATA}` - Path to the plugin data directory
...
| Use absolute paths with `$CURSOR_PROJECT_DIR` | Hardcode paths |
...
Run Cursor with debug flag for detailed hook logs:
...
Use `Ctrl+O` in Cursor to enable verbose mode and see hook execution progress.

### Test Hooks Independently

```bash
# Test with sample JSON input
echo '{"tool_name": "Bash", "tool_input": {"command": "ls -la"}}' | python3 .cursor/hooks/validate-bash.py
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "\"$CURSOR_PROJECT_DIR/.cursor/hooks/format-code.sh\"",
            "timeout": 30
          },
          {
            "type": "command",
            "command": "python3 \"$CURSOR_PROJECT_DIR/.cursor/hooks/security-scan.py\"",
            "timeout": 10
          }
        ]
      }
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "python3 \"$CURSOR_PROJECT_DIR/.cursor/hooks/validate-prompt.py\""
          }
        ]
      }
    ],
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "\"$CURSOR_PROJECT_DIR/.cursor/hooks/session-init.sh\""
          }
        ]
      }
    ],
...
| **Environment** | Runs in current directory with Cursor's environment |
...
- Run `cursor --debug` to see hook execution logs
...
Edit `~/.cursor/settings.json` or `.cursor/settings.json` with the hook configuration shown above.
...
- **[Advanced Features](../09-advanced-features/)** - Explore advanced Cursor capabilities

## Additional Resources

- **[Official Hooks Documentation](https://docs.cursor.com/docs/en/hooks)** - Complete hooks reference
