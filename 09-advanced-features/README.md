...
Advanced features in Cursor extend the core capabilities with planning, reasoning, automation, and control mechanisms. These features enable sophisticated workflows for complex development tasks, code review, automation, and multi-session management.
...
- **Permission Modes**: Control what Cursor can do (`default`, `acceptEdits`, `plan`, `auto`, `dontAsk`, `bypassPermissions`)
- **Print Mode**: Run Cursor non-interactively for automation and CI/CD (`cursor -p`)
...
- **Remote Control**: Control Cursor from Cursor.ai or the Cursor app
- **Web Sessions**: Run Cursor in the browser at cursor.com/code
...
Planning mode allows Cursor to think through complex tasks before implementing them, creating a detailed plan that you can review and approve.

### What is Planning Mode?

Planning mode is a two-phase approach:
1. **Planning Phase**: Cursor analyzes the task and creates a detailed implementation plan
2. **Implementation Phase**: After approval, Cursor executes the plan
...
Cursor: I'll create a plan for implementing real-time notifications.
...
Cursor: [Begins implementation following the plan]
```
...
Cursor: [Implements modified plan]
```
...
```bash
cursor --model opusplan "design and implement the new API"
```
...
Extended thinking allows Cursor to spend more time reasoning about complex problems before providing a solution.

### What is Extended Thinking?

Extended thinking is a deliberate, step-by-step reasoning process where Cursor:
...
**CLI flag**:
```bash
cursor --effort high "complex architectural review"
```
...
> **Note:** The keyword "ultrathink" in prompts activates deep reasoning mode. Effort levels `low`, `medium`, `high`, and `max` (Opus 4.6 only) control how much reasoning Cursor performs.
...
Cursor: Let me think through this architectural decision carefully...
...
export CURSOR_EFFORT_LEVEL=high
```
...
Auto Mode is a Research Preview permission mode (March 2026) that uses a background safety classifier to review each action before execution. It allows Cursor to work autonomously while blocking dangerous operations.

### Requirements

- **Plan**: Team plan (Enterprise and API rolling out)
- **Model**: your chosen model or Opus 4.6
- **Classifier**: Runs on your chosen model (adds extra token cost)

### Enabling Auto Mode

```bash
# Unlock auto mode with CLI flag
cursor --enable-auto-mode
...
Cursor: Starting tests in background (task-id: bg-1234)
...
Cursor: [Works on auth module while tests run in background]

[Later, when tests complete]

Cursor: 📢 Background task bg-1234 completed:
✅ 245 tests passed
❌ 3 tests failed
...
Cursor: Starting build... (task-id: bg-5001)

User: Also run the linter in background

Cursor: Starting linter... (task-id: bg-5002)

User: While those run, let's implement the new API endpoint

Cursor: [Implements API endpoint while build and linter run]

[10 minutes later]

Cursor: 📢 Build completed successfully (bg-5001)
📢 Linter found 12 issues (bg-5002)

User: Show me the linter issues

Cursor: [Shows linter output from bg-5002]
```
...
Scheduled Tasks let you run prompts automatically on a recurring schedule or as one-time reminders. Tasks are session-scoped — they run while Cursor is active and are cleared when the session ends. Available since v2.1.72+.
...
- Tasks only fire while Cursor is running — no catch-up for missed fires
...
| **Missed fires** | No catch-up — skipped if Cursor was not running |
| **Persistence** | Not persisted across restarts |

### Cloud Scheduled Tasks

Use `/schedule` to create Cloud scheduled tasks that run on Cursor cloud infrastructure:

```
/schedule daily at 9am run the test suite and report failures
```

Cloud scheduled tasks persist across restarts and do not require Cursor to be running locally.

### Disabling scheduled tasks

```bash
export CURSOR_DISABLE_CRON=1
```
...
Permission modes control what actions Cursor can take without explicit approval.
...
#### Default Mode
Cursor asks for confirmation on significant actions:

```
User: Fix the bug in auth.ts

Cursor: I need to modify src/auth.ts to fix the bug.
...
Cursor: I'll create a plan for implementing authentication.
...
Cursor: [Makes changes without asking]
```
...
**Code Review**:
```
User: cursor --permission-mode plan
User: Review this PR and suggest improvements

Cursor: [Reads code, provides feedback, but cannot modify]
```

**Pair Programming**:
```
User: cursor --permission-mode default
User: Let's implement the feature together

Cursor: [Asks for approval before each change]
```

**Automated Tasks**:
```
User: cursor --permission-mode acceptEdits
User: Fix all linting issues in the codebase

Cursor: [Auto-accepts file edits without asking]
```
...
Print mode (`cursor -p`) allows Cursor to run without interactive input, perfect for automation and CI/CD. This is the non-interactive mode, replacing the older `--headless` flag.
...
```bash
# Run specific task
cursor -p "Run all tests"

# Process piped content
cat error.log | cursor -p "Analyze these errors"

# CI/CD integration (GitHub Actions)
- name: AI Code Review
  run: cursor -p "Review PR"
```
...
cursor -p "Run all tests and generate coverage report"

# With structured output
cursor -p --output-format json "Analyze code quality"

# With input from stdin
echo "Analyze code quality" | cursor -p "explain this"
```
...
Print mode (`cursor -p`) supports several flags for automation:
...
cursor -p --output-format json "analyze this codebase"

# With schema validation
cursor -p --json-schema '{"type":"object","properties":{"issues":{"type":"array"}}}' \
  "find bugs in this code"

# Disable session persistence
cursor -p --no-session-persistence "one-off analysis"
```
...
Manage multiple Cursor sessions effectively.
...
| `cursor -c` | Continue most recent conversation |
| `cursor -r "session"` | Resume session by name or ID |
...
```bash
cursor -r "auth-refactor" "finish this PR"
```
...
Or from the CLI:
```bash
cursor --resume auth-refactor --fork-session "try OAuth instead"
```
...
cursor -r "auth-refactor"

# Resume and fork for experimentation
cursor --resume auth-refactor --fork-session "alternative approach"
```
...
Cursor supports keyboard shortcuts for efficiency. Here's the complete reference from official docs:
...
Create custom keyboard shortcuts by running `/keybindings`, which opens `~/.cursor/keybindings.json` for editing (v2.1.18+).
...
Cursor provides intelligent tab completion:
...
Cursor: [Processes the multi-line request]
```
...
Voice Dictation provides push-to-talk voice input for Cursor, allowing you to speak your prompts instead of typing them.
...
| **Account requirement** | Requires a Cursor.ai account for STT processing |

### Configuration

Customize the push-to-talk keybinding in your keybindings file (`/keybindings`). Voice dictation uses your Cursor.ai account for speech-to-text processing.

---

## Channels

Channels (Research Preview) allow MCP servers to push messages into running Cursor sessions, enabling real-time integrations with external services.

### Subscribing to Channels

```bash
# Subscribe to channel plugins at startup
cursor --channels discord,telegram
```
...
2. Incoming messages are pushed into the active Cursor session
3. Cursor can read and respond to messages within the session context
...
Chrome Integration connects Cursor to your Chrome or Microsoft Edge browser for live web automation and debugging. This is a beta feature available since v2.0.73+ (Edge support added in v1.0.36+).
...
```bash
cursor --chrome      # Enable Chrome connection
cursor --no-chrome   # Disable Chrome connection
```
...
Select "Enabled by default" to activate Chrome Integration for all future sessions. Cursor shares your browser's login state, so it can interact with authenticated web apps.
...
The Chrome extension manages per-site access. Grant or revoke access for specific sites at any time through the extension popup. Cursor only interacts with sites you have explicitly allowed.

### How it works

Cursor controls the browser in a visible window — you can watch actions happen in real time. When the browser encounters a login page or CAPTCHA, Cursor pauses and waits for you to handle it manually before continuing.
...
Remote Control lets you continue a locally running Cursor session from your phone, tablet, or any browser. Your local session keeps running on your machine — nothing moves to the cloud. Available on Pro, Max, Team, and Enterprise plans (v2.1.51+).
...
cursor remote-control --name "Auth Refactor"
```
...
3. **Find by name** — Browse your sessions at cursor.com/code or in the Cursor mobile app (iOS/Android)
...
### Remote Control vs Cursor on the web

| Aspect | Remote Control | Cursor on Web |
|--------|---------------|-------------------|
| **Execution** | Runs on your machine | Runs on Cursor cloud |
...
- One remote session per Cursor instance
- Terminal must stay open on the host machine
- Session times out after ~10 minutes if the network is unreachable

### Use cases

- Control Cursor from a mobile device or tablet while away from your desk
- Use the richer cursor.com UI while maintaining local tool execution
...
Web Sessions allow you to run Cursor directly in the browser at cursor.com/code, or create web sessions from the CLI.

### Creating a Web Session

```bash
# Create a new web session from the CLI
cursor --remote "implement the new API endpoints"
```

This starts a Cursor session on cursor.com that you can access from any browser.
...
The Cursor Desktop App provides a standalone application with visual diff review, parallel sessions, and integrated connectors. Available for macOS and Windows (Pro, Max, Team, and Enterprise plans).

### Installation

Download from [cursor.com](https://cursor.com) for your platform:
- **macOS**: Universal build (Apple Silicon and Intel)
- **Windows**: x64 and ARM64 installers available

See the [Desktop Quickstart](https://docs.cursor.com/docs/en/desktop-quickstart) for setup instructions.
...
| Feature | Description |
|---------|-------------|
| **Diff view** | File-by-file visual review with inline comments; Cursor reads comments and revises |
...
Configure dev server behavior in `.cursor/launch.json`:
...
- **Remote sessions**: Run on Cursor cloud infrastructure; continue even when the app is closed. Accessible from [cursor.com](https://cursor.com) or the Cursor mobile app
- **SSH sessions**: Connect to remote machines over SSH with full access to the remote filesystem and tools. Cursor must be installed on the remote machine
...
Use the `CURSOR_TASK_LIST_ID` environment variable to create named task directories shared across sessions:

```bash
export CURSOR_TASK_LIST_ID=my-project-sprint-3
```
...
```bash
export CURSOR_ENABLE_PROMPT_SUGGESTION=false
```
...
Git Worktrees allow you to start Cursor in an isolated worktree, enabling parallel work on different branches without stashing or switching.

### Starting in a Worktree

```bash
# Start Cursor in an isolated worktree
cursor --worktree
# or
cursor -w
```
...
Sandboxing provides OS-level filesystem and network isolation for Bash commands executed by Cursor. This is complementary to permission rules and provides an additional security layer.
...
**CLI flags**:
```bash
cursor --sandbox       # Enable sandboxing
cursor --no-sandbox    # Disable sandboxing
```
...
Managed Settings enable enterprise administrators to deploy Cursor configuration across an organization using platform-native management tools.
...
<dict>
  ...
  <array>
    <string>cursor-sonnet-4-6</string>
    <string>cursor-haiku-4-5</string>
  </array>
</dict>
</plist>
```
...
1. **Global config**: `~/.cursor/config.json`
2. **Project config**: `./.cursor/config.json`
3. **User config**: `~/.config/cursor-code/settings.json`
...
```json
{
  ...
  "hooks": {
    ...
    "PostToolUse:Write": "~/.cursor/hooks/security-scan.sh"
  },
  "mcp": {
    "enabled": true,
    "servers": {
...
```json
{
  ...
  "hooks": {
    ...
    "PostToolUse": [{ "matcher": "Write", "hooks": ["~/.cursor/hooks/security-scan.sh"] }],
    "Stop": [{ "hooks": ["~/.cursor/hooks/notify.sh"] }]
  },
...
```bash
# Model selection
export ANTHROPIC_MODEL=cursor-opus-4-6
export ANTHROPIC_DEFAULT_OPUS_MODEL=cursor-opus-4-6
export ANTHROPIC_DEFAULT_SONNET_MODEL=cursor-sonnet-4-6
export ANTHROPIC_DEFAULT_HAIKU_MODEL=cursor-haiku-4-5
...
# Feature toggles
export CURSOR_DISABLE_AUTO_MEMORY=true
export CURSOR_DISABLE_BACKGROUND_TASKS=true
export CURSOR_DISABLE_CRON=1
export CURSOR_DISABLE_GIT_INSTRUCTIONS=true
export CURSOR_DISABLE_TERMINAL_TITLE=true
export CURSOR_DISABLE_1M_CONTEXT=true
export CURSOR_DISABLE_NONSTREAMING_FALLBACK=true
export CURSOR_ENABLE_PROMPT_SUGGESTION=false
export CURSOR_ENABLE_TASKS=true
export CURSOR_SIMPLE=true              # Set by --bare flag
...
# Task management
export CURSOR_TASK_LIST_ID=my-project-tasks

# Agent teams (experimental)
export CURSOR_EXPERIMENTAL_AGENT_TEAMS=true

# Subagent and plugin configuration
export CURSOR_SUBAGENT_MODEL=sonnet
export CURSOR_PLUGIN_SEED_DIR=./my-plugins
export CURSOR_NEW_INIT=true

# Subprocess and streaming
export CURSOR_SUBPROCESS_ENV_SCRUB="SECRET_KEY,DB_PASSWORD"
export CURSOR_AUTOCOMPACT_PCT_OVERRIDE=80
export CURSOR_STREAM_IDLE_TIMEOUT_MS=30000
...
For more information about Cursor and related features:

- [Official Interactive Mode Documentation](https://docs.cursor.com/docs/en/interactive-mode)
- [Official Headless Mode Documentation](https://docs.cursor.com/docs/en/headless)
- [CLI Reference](https://docs.cursor.com/docs/en/cli-reference)
...
- [Official Scheduled Tasks Documentation](https://docs.cursor.com/docs/en/scheduled-tasks)
- [Official Chrome Integration Documentation](https://docs.cursor.com/docs/en/chrome)
- [Official Remote Control Documentation](https://docs.cursor.com/docs/en/remote-control)
- [Official Keybindings Documentation](https://docs.cursor.com/docs/en/keybindings)
- [Official Desktop App Documentation](https://docs.cursor.com/docs/en/desktop)
