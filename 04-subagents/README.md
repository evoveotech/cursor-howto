...
Subagents are specialized AI assistants that Cursor can delegate tasks to. Each subagent has a specific purpose, uses its own context window separate from the main conversation, and can be configured with specific tools and a custom system prompt.
...
14. [`cursor agents` CLI Command](#cursor-agents-cli-command)
...
Subagents enable delegated task execution in Cursor by:
...
| 2 | **Project subagents** | `.cursor/agents/` | Current project |
| 3 | **User subagents** | `~/.cursor/agents/` | All projects |
...
2. **Project-level** - `.cursor/agents/` (current project)
3. **User-level** - `~/.cursor/agents/` (all projects)
...
Cursor includes several built-in subagents that are always available:
...
| **Cursor Guide** | Haiku | Answer Cursor feature questions |
...
**When used**: When Cursor needs to understand the codebase before presenting a plan.
...
| **Purpose** | Configure the Cursor status line display |

**When used**: When setting up or customizing the status line.

### Cursor Guide Subagent

| Property | Value |
|----------|-------|
| **Model** | Haiku (fast, low-latency) |
| **Tools** | Read-only |
| **Purpose** | Answer questions about Cursor features and usage |

**When used**: When users ask questions about how Cursor works or how to use specific features.
...
cat > .cursor/agents/test-runner.md << 'EOF'
---
name: test-runner
...
Cursor proactively delegates tasks based on:
...
Use the `cursor agents` command to list all configured agents from all sources:
...
|-------|-----------|----------|
| `user` | `~/.cursor/agent-memory/<name>/` | Personal notes and preferences across all projects |
| `project` | `.cursor/agent-memory/<name>/` | Project-specific knowledge shared with the team |
| `local` | `.cursor/agent-memory-local/<name>/` | Local project knowledge not committed to version control |
...
```bash
export CURSOR_DISABLE_BACKGROUND_TASKS=1
```
...
## `cursor agents` CLI Command

The `cursor agents` command lists all configured agents grouped by source (built-in, user-level, project-level):
...
Agent Teams coordinate multiple Cursor instances working together on complex tasks. Unlike subagents (which are delegated subtasks returning results), teammates work independently with their own context and communicate directly through a shared mailbox system.

> **Note**: Agent Teams is experimental and requires Cursor v2.1.32+. Enable it before use.
...
```bash
export CURSOR_EXPERIMENTAL_AGENT_TEAMS=1
```
...
{
  "env": {
    "CURSOR_EXPERIMENTAL_AGENT_TEAMS": "1"
  }
}
```
...
Once enabled, ask Cursor to work with teammates in your prompt:
...
Cursor will create the team, assign tasks, and coordinate the work automatically.
...
Team configurations are stored at `~/.cursor/teams/{team-name}/config.json`.
...
- **Team Lead**: The main Cursor session that creates the team, assigns tasks, and coordinates
...
- **Teammates**: Independent Cursor instances, each with their own context window
...
- **Transcripts** - Subagent transcripts are stored at `~/.cursor/projects/{project}/{sessionId}/subagents/agent-{agentId}.jsonl`
- **Auto-compaction** - Subagent context auto-compacts at ~95% capacity (override with `CURSOR_AUTOCOMPACT_PCT_OVERRIDE` environment variable)
...
**Do:**
- Start with Cursor-generated agents - Generate initial subagent with Cursor, then iterate to customize
...
cp /path/to/04-subagents/*.md .cursor/agents/

# Remove the README (not needed in .cursor/agents)
rm .cursor/agents/README.md
```
...
# Copy agents
cp /path/to/04-subagents/code-reviewer.md ~/.cursor/agents/
cp /path/to/04-subagents/debugger.md ~/.cursor/agents/
...
- [Official Subagents Documentation](https://docs.cursor.com/docs/en/sub-agents)
- [CLI Reference](https://docs.cursor.com/docs/en/cli-reference) - `--agents` flag and other CLI options
...
*This guide covers complete subagent configuration, delegation patterns, and best practices for Cursor.*
